...
| 2b | Vanilla coverage | **As implemented:** `codecov.yml` deleted (the action had already gone in 2a); floors in `vite.config.ts` as a merge gate; coverage table to `$GITHUB_STEP_SUMMARY`; PR comment via `actions/github-script`. Executed last rather than second, so the floors are seeded *after* #51 brought the Normality tab under test — higher than they would have been in sequence. | Floors seeded at 99.7 / 91.51 / 99.05 / 99.7; a deliberate drop proven to fail CI |
...
- ~~No new runtime dependency that phones home. The target must survive
  ...
  > with no `content-security-policy` header at all. That was this
